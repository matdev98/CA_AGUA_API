...
namespace caAGUAAPI.API.Controllers
{
    ...
    public class CuotasPlanController : ControllerBase
    {
        private readonly ILogger<CuotasPlanController> _logger;
        private readonly IMapper _mapper;

        private readonly IBaseService<CuotasPlan> _baseService;

        public CuotasPlanController(IBaseService<CuotasPlan> baseService, ILogger<CuotasPlanController> logger, IMapper mapper)
        {
        ...
        [HttpGet]
        [ProducesResponseType(typeof(ResultadoDTO<IEnumerable<CuotasPlan>>), StatusCodes.Status200OK)]
        public async Task<ActionResult<ResultadoDTO<IEnumerable<CuotasPlan>>>> GetAllMunicipios()
        {
            _logger.LogInformation("Obteniendo todos las auditorias");

            var resultado = await _baseService.GetAllAsync();
            var resultadoMapeado = _mapper.Map<IEnumerable<CuotasPlan>>(resultado);

            var resultadoDTO = ResultadoDTO<IEnumerable<CuotasPlan>>.Exitoso(resultadoMapeado, "Listado de auditorias obtenido correctamente");

            return Ok(resultadoDTO);
        }

        [HttpGet("{id}")]
        [ProducesResponseType(typeof(ResultadoDTO<CuotasPlan>), StatusCodes.Status200OK)]
        public async Task<ActionResult<ResultadoDTO<CuotasPlan>>> GetById(int id)
        {
            ...
            if (resultado == null)
                return NotFound(ResultadoDTO<CuotasPlan>.Fallido($"No se encontró la auditoria con ID {id}"));

            var resultadoMapeado = _mapper.Map<CuotasPlan>(resultado);
            var resultadoDTO = ResultadoDTO<CuotasPlan>.Exitoso(resultadoMapeado, "Auditoria encontrada correctamente");

            return Ok(resultadoDTO);
        }

        [HttpPost]
        [ProducesResponseType(typeof(ResultadoDTO<CuotasPlan>), StatusCodes.Status201Created)]
        public async Task<ActionResult<ResultadoDTO<CuotasPlan>>> Create([FromBody] CuotasPlanDTO dto)
        {
            _logger.LogInformation("Creando una nueva auditoria");

            var entity = _mapper.Map<CuotasPlan>(dto);
            var createdEntity = await _baseService.AddAsync(entity);
            var resultadoMapeado = _mapper.Map<CuotasPlan>(createdEntity);

            var resultadoDTO = ResultadoDTO<CuotasPlan>.Exitoso(resultadoMapeado, "Auditoria creada exitosamente");
        ...
        public async Task<ActionResult<ResultadoDTO<string>>> Update(int id, [FromBody] CuotasPlanDTO dto)
        {
